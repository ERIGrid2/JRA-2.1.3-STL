...
def connectEntities(world, entities):
    ...
    world.connect(entities['dh_network'], entities['hex_consumer1'], ('T_supply_cons1', 'T_supply'), 'initialized',
                  weak=True, initial_data={'T_supply_cons1': 70, 'initialized': initialized})
    ...
    world.connect(entities['dh_network'], entities['hex_consumer2'], ('T_supply_cons2', 'T_supply'), 'initialized',
                  weak=True, initial_data={'T_supply_cons2': 70, 'initialized': initialized})

    # Heat pump.
    world.connect(entities['flex_heat_ctrl'], entities['heat_pump'], ('mdot_2_return', 'mdot_evap_in'),
                  weak=True, initial_data={'mdot_evap_in': 0})
    world.connect(entities['dh_network'], entities['heat_pump'], ('T_evap_in', 'T_evap_in'), 'initialized',
                  weak=True, initial_data={'T_evap_in': 40, 'initialized': initialized})
    world.connect(entities['flex_heat_ctrl'], entities['heat_pump'], ('Q_HP_set', 'Q_set'),
                  weak=True, initial_data={'Q_set': 0})
    ...
    world.connect(entities['flex_heat_ctrl'], entities['storage_tank'], ('mdot_3_supply', 'mdot_dis_out'),
                  weak=True, initial_data={'mdot_3_supply': 0})
    world.connect(entities['dh_network'], entities['storage_tank'], ('T_return_tank', 'T_dis_in'), 'initialized',
                  weak=True, initial_data={'T_return_tank': INIT_STORAGE_TANK_TEMP, 'initialized': initialized})
    # was really weak
...
if __name__ == '__main__':
    ...
    world_args = {'max_loop_iterations': 2000}
    ...
    # Run the simulation.
    world.run(until = end, lazy_stepping=True)

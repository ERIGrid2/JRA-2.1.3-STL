...
META = {
    ...
    'models': {
        'ConstantTcondHP': {
            ...
            # comment out the following two lines to not enter same time loop for initialization
            'trigger': ['T_evap_in', 'mdot_cond_in', 'mdot_evap_in', 'Q_set'],
            'non-persistent': ['initialized', 'T_cond_out_target', 'T_cond_in', 'T_evap_in', 'mdot_cond_out', 'T_cond_out']
        },
    },
}
...
class ConstantTcondHPSimulator(Simulator):
    ...
    def __init__(self, META=META):
        ...
        self.input_vars = {'T_cond_in', 'T_evap_in', 'Q_set', 'mdot_cond_in', 'mdot_evap_in', 'opmode', 'initialized'}
    ...
    def step(self, time, inputs, max_advance):
        # if time < 200:
        #     print('Heat Pump step: %s - %s' % (time, inputs))
    ...
    def get_data(self, outputs):
        if self.last_time == 0 and not self.init_finished:
        ...
        #     print('Heat pump get_data: %s' % data)
